...
from patchright._impl._element_handle import ElementHandle, determine_screenshot_type
...
class Page(ChannelOwner):
    ...
    async def evaluate(
        ...
        *,
        isolatedContext: Optional[bool] = True,
    ) -> Any:
        return await self._main_frame.evaluate(
            expression, arg, isolatedContext=isolatedContext
        )
    ...
    async def set_viewport_size(self, viewportSize: ViewportSize) -> None:
        ...
        await self._channel.send("setViewportSize", None, locals_to_params(locals()))
    ...
    async def route_web_socket(
        ...
        self._web_socket_routes.insert(
            0, WebSocketRouteHandler(self._browser_context._base_url, url, handler)
        )
    ...
    def expect_request(
        ...
        def my_predicate(request: Request) -> bool:
            if not callable(urlOrPredicate):
                return url_matches(
                    self._browser_context._base_url, request.url, urlOrPredicate
                )
        ...
        for uid, data in self._locator_handlers.copy().items():
            if data.locator._equals(locator):
                ...
                self._channel.send_no_reply(
                    "unregisterLocatorHandler", None, {"uid": uid}
                )

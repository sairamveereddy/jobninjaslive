...
import patchright
import patchright._impl._impl_to_api_mapping
...
class RootChannelOwner(ChannelOwner):
    ...
    async def initialize(self) -> "Playwright":
        return from_channel(
            await self._channel.send("initialize", None, {"sdkLanguage": "python"})
        )
...
def _extract_stack_trace_information_from_stack(
    ...
    for frame in st:
        if patchright._impl._impl_to_api_mapping.__file__ == frame.filename:
            continue
